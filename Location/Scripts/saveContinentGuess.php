...
if (isset($_COOKIE[$cookieName])) {
    ...
    $stmt = $mysqli->prepare("SELECT COUNT(*) FROM user_location_data WHERE user_id = ? AND date = ?");
    ...
    if ($count == 0) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO user_location_data (user_id, date, guessed_continents) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $id, $currentDate, $jsonGuess);
        $stmt->execute();
        $stmt->close();

    } else {
        $stmt = $mysqli->prepare("SELECT guessed_continents FROM user_location_data WHERE user_id = ? AND date = ?");
        $stmt->bind_param("ss", $id, $currentDate);
        $stmt->execute();
        ...
        if (!in_array($guess, $existingGuesses)) {
            ...
            $update = $mysqli->prepare("UPDATE user_location_data SET guessed_continents = ? WHERE user_id = ? AND date = ?");
