...
if (isset($_COOKIE[$cookieName])) {
    ...
    $stmt = $mysqli->prepare("SELECT score, guessed_classes, guessed_skill FROM user_ability_data WHERE user_id = ? AND date = ?");
    $stmt->bind_param("ss", $id, $currentDate);
    $stmt->execute();
    $stmt->bind_result($score, $guessedClasses, $guessedSkill);
    $stmt->fetch();
    $stmt->close();
